...
<div id="results-container" class="mt-12 min-h-[100px]">
    <div id="empty-state" class="hidden flex flex-col items-center justify-center py-10 border-2 border-dashed border-slate-100 dark:border-slate-800 rounded-3xl">
        <i class="fas fa-chart-bar text-4xl text-slate-200 dark:text-slate-700 mb-4"></i>
        <p class="text-slate-400 dark:text-slate-500 text-sm">Search for teams to generate professional AI analysis</p>
    </div>

    <div id="loading-state" class="hidden flex-col items-center justify-center space-y-6 py-10">
         ...
         <p class="text-slate-500 dark:text-slate-400 animate-pulse text-center text-sm">
...
<script>
    ...
    if (form) form.addEventListener('submit', (e) => {
        ...
        fetch(`api/get_prediction.php?home=${encodeURIComponent(home)}&away=${encodeURIComponent(away)}${visitorParam}`)
            ...
            .then(data => {
                if (data.error) {
                    if (data.error === 'limit_exceeded' || data.error === 'insufficient_credits') {
                ...
                if (activeVisitor) {
                    const charge = <?php echo $settings['prediction_charge'] ?? 0.03; ?>;
                    activeVisitor.credits -= charge;
                    if (typeof updateUIForVisitor === 'function') updateUIForVisitor();
                }
        ...
        input.addEventListener('input', () => {
            ...
            debounceTimer = setTimeout(() => {
                fetch(`api/get_teams.php?q=${encodeURIComponent(val)}`)
                    ...
                    .then(data => {
                        if (data.length > 0) {
                            suggestionsList.innerHTML = data.map(item => `
                                <li class="px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-700 cursor-pointer border-b border-slate-100 dark:border-slate-700 last:border-0 transition-colors text-left" onclick="selectTeam('${input.id}', '${item.name.replace(/'/g, "\\'")}')">
                    ...
                    .finally(() => {
                        input.parentElement.classList.remove('opacity-70');
                    });
            }, 150);
        });
    ...
    function renderPrediction(data, home, away) {
        ...
        predictionResult.innerHTML = `
            <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl overflow-hidden shadow-2xl animate-in fade-in slide-in-from-bottom-6 duration-1000 ease-out mt-8">
                ...
                <div class="p-8 md:p-12 space-y-12 text-left">
